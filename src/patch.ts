...
import { ChatCompletion, ChatCompletionCreateParams, ChatCompletionMessage, Completions } from "openai/resources";
...
export class OpenAISchema {
  ...
  get openai_schema() {
    return {
      ...
      description: this.response_model['description'] || `Correctly extracted \`${this.response_model['title'] || 'schema'}\` with all the required parameters with correct types`,
      parameters: Object.keys(this.response_model).reduce((acc, curr) => {
        if (curr.startsWith('$') || ['title', 'description', 'additionalProperties'].includes(curr)) return acc;
...
type PatchedChatCompletionCreateParams = ChatCompletionCreateParams & { response_model?: ZodSchema | OpenAISchema, max_retries?: number };


function handleResponseModel(response_model: ZodSchema | OpenAISchema, args: PatchedChatCompletionCreateParams[], mode: MODES = MODES.FUNCTIONS): [OpenAISchema, PatchedChatCompletionCreateParams[], MODES] {
...
function processResponse(response: OpenAI.Chat.Completions.ChatCompletion, response_model: OpenAISchema, mode: MODES = MODES.FUNCTIONS) {
  const message = response.choices[0].message;
  if (mode === MODES.FUNCTIONS) {
    assert.equal(message.function_call!.name, response_model.openai_schema.name, "Function name does not match")
    return response_model.zod_schema.parse(JSON.parse(message.function_call!.arguments));
  } else if (mode === MODES.TOOLS) {
    const tool_call = message.tool_calls![0]
    assert.equal(tool_call.function.name, response_model.openai_schema.name, "Tool name does not match")
...
function dumpMessage(message: ChatCompletionMessage) {
  const ret: ChatCompletionMessage = { role: message.role, content: message.content || '' }
...
export const patch = ({ client, mode }: { client: OpenAI, response_model?: ZodSchema | OpenAISchema, max_retries?: number, mode?: MODES }): OpenAI => {
  client.chat.completions.create = new Proxy(client.chat.completions.create, {
    async apply(target, ctx, args: PatchedChatCompletionCreateParams[]) {
      let retries = 0, max_retries = args[0].max_retries || 1, response: ChatCompletion | undefined = undefined, response_model = args[0].response_model;
      ...
      while (retries < max_retries) {
        try {
          response = await target.apply(ctx, args as [PatchedChatCompletionCreateParams]) as ChatCompletion;
